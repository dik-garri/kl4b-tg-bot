...
Команды доступны только админам (колонка `role = admin` в листе `members`).

| Команда | Описание |
|---------|----------|
| `/report` | Сгенерировать отчёт за последнюю полную неделю |
| `/report 2026-W08` | Отчёт за конкретную неделю |

Отчёт приходит **в личку** админу. Бот не пишет в групповые чаты.
