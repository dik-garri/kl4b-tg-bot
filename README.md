...
- Каждое воскресенье постит CSV-отчёт и ссылку на таблицу в топик "Объявления"
...
│   ├── Commands.gs              # Команды бота (/report, /getlastreport, /help)
...
Команды только для админов (колонка `role = admin` в листе `members`):

| Команда | Описание |
|---------|----------|
| `/report` | Сгенерировать отчёт за последнюю полную неделю (обновляет страйки и трофеи) |
| `/report 2026-W08` | Отчёт за конкретную неделю |
| `/getlastreport` | Получить последний готовый отчёт без пересчёта |

Отчёт приходит **в личку** админу: CSV-файл + ссылка на Google Sheet. Бот не пишет в групповые чаты.

**Важно:** админ должен сначала написать боту `/start` в личных сообщениях, иначе Telegram не разрешит боту отправить DM.

### Настройка админа

1. Открыть Google Spreadsheet → лист `members`
2. Найти участника
3. В столбце `role` вписать `admin`
